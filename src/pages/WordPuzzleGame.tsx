...
export default function WordPuzzleGame() {
  ...
  // 🎯 Current Word
  const [targetWord, setTargetWord] = useState("PLANET");
    ...
    while (!placed) {
      ...
      if (fits) {
        for (let i = 0; i < randomWord.length; i++) {
          ...
          newGrid[newRow][newCol] = randomWord[i];
        }
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (row === undefined || col === undefined || !letter) {
      return;
    }
    ...
    const alreadySelected = selectedCells.some(
      (cell) => cell.row === rowNum && cell.col === colNum,
    );
  ...
  const checkWord = () => {
    ...
    if (
      selected === targetWord ||
      selected === targetWord.split("").reverse().join("")
    ) {
  ...
  useEffect(() => {
    if (score >= 70) {
      ...
      setUser((prev) => ({
        ...
        completedMissions: [...prev.completedMissions, 202],
      }));
