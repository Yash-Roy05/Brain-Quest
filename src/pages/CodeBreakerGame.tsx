...
export default function CodeBreakerGame() {
  ...
  // 🧠 Smart Hints
  const [smartHints, setSmartHints] =
    useState<string[]>([]);
  ...
  const checkGuess = () => {
    ...
    const resultMessage =
      `✅ ${correctPosition} Correct Position | 🔄 ${correctDigit} Correct But Wrong Place`;
  ...
  return (
    ...
    <>
      ...
      <PageWrapper>
        ...
        <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-black flex items-center justify-center p-6 relative overflow-hidden">
          ...
          <div className="absolute bottom-10 right-10 text-7xl animate-bounce">
            🔐
          </div>
          ...
          <div className="bg-white rounded-[40px] shadow-2xl p-8 md:p-10 w-full max-w-3xl text-center relative z-10">
              ...
              <div className="space-y-3">
                ...
                {smartHints.map(
                  (item, index) => (
                    ...
                    <div
                      key={index}
                      className="bg-white rounded-2xl px-4 py-3 font-bold text-gray-700 shadow"
                    >
                      {item}
                    </div>
            ...
            <input
              ...
              className="w-full border-4 border-gray-300 focus:border-green-500 outline-none rounded-2xl px-6 py-5 text-3xl font-bold text-center mb-6 tracking-[15px]"
            />

            {/* Check Button */}
            <button
              onClick={checkGuess}
              className="bg-green-500 hover:bg-green-600 hover:scale-105 active:scale-95 transition duration-300 text-white text-2xl font-bold px-10 py-4 rounded-2xl shadow-xl mb-8"
            >
              Crack Code 🚀
            </button>
