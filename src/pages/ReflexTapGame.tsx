...
export default function ReflexTapGame() {
  ...
  // 🎨 Target Type
  const [targetType, setTargetType] = useState<"good" | "bad">("good");
  ...
  // ⚡ Speed
  const speed = Math.max(400, 1200 - score * 8);

  // 🚀 Move Target
  const moveTarget = () => {
    const randomTop = Math.random() * 60;

    const randomLeft = Math.random() * 65;
    ...
    // 🎨 Random Type
    const randomType = Math.random() > 0.25 ? "good" : "bad";
  ...
  }, [score, showTarget, targetType, won, gameOver]);
  ...
  return (
    ...
    "
        ...
        <div className="h-screen overflow-hidden bg-gradient-to-br from-black via-purple-950 to-black overflow-hidden relative">
          {/* Header */}
